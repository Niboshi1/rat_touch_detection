...
for trial_name in trial_names:
    ...
    try:
        ...
        onset_savedir = os.path.join(
            "/".join(dat_path.split("/")[:-3]), "events/Acquisition_Board-100.Rhythm Data/")
        ...
        samp_rate = int(cfg.extraction.dat.sampling_rate /
                        cfg.extraction.dat.downsampling_ratio)
        ...
        assert len(trigger_segments) == len(step_onset_segments), f"dat-segments and video-numbers does not match ({len(trigger_segments)}, {len(step_onset_segments)})"
        ...
        for trigger_seg, step_onset_seg in zip(trigger_segments, step_onset_segments):
            if len(trigger_seg) > max(step_onset_seg):
                ...
                cut_step_onset_seg = step_onset_seg[np.where(step_onset_seg < len(trigger_seg))]
        ...
        with h5py.File(onset_h5file, "w") as h5:
            h5.create_dataset(
                name="step_idx", data=onsets)
            h5.create_dataset(
                name="texture", data=texture)
